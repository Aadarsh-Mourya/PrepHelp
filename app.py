...
def pred():
    if 'resume' in request.files and request.files['resume'].filename != '':
        ...
        num_questions = 5  
        questions = generate_questions(extracted_skills, num_questions)

        if not questions:
            questions = [{"question": "No relevant questions found based on the uploaded resume.", "answer": ""}]

        session['extracted_skills'] = extracted_skills
        session['extracted_education'] = extracted_education
        ...
        session.pop('extracted_education', None)
...
def generate_questions(skills, num_questions=5):
    ...
    for skill in skills:
        if skill in skills_data:
            skill_questions = skills_data[skill]
            for q in skill_questions:
                if q["question"] not in seen_questions:
                    
                    questions.append({"question": q["question"], "answer": q.get("answer", "")})
...
def get_user_answer(question_index):
    ...
    return request.form.get(f'answer_{question_index}', "") 
...
def evaluate():
    ...
    for index, question in enumerate(questions, start=1): 
        user_answer = get_user_answer(index)  
        correct_answer = get_correct_answer(question)  
        similarity_score = check_similarity(user_answer, correct_answer)  
        feedback.append({
            'question': question,
            'user_answer': user_answer,
            'correct_answer': correct_answer,
            'similarity_score': similarity_score,
            'feedback': 'Correct' if similarity_score >= 0.7 else 'Incorrect'   
        })

    return render_template("feedback.html", feedback=feedback)

def load_questions():
    with open('skills_questions.json') as f:
        data = json.load(f)
...
def questions():
    ...
    if not questions:
        return render_template('questions.html', questions=None, feedback="No questions available. Please upload a resume to generate questions.")
    
    
    return render_template('questions.html', questions=questions, feedback=None)
...
def feedback():
    ...
    if not asked_questions:
        return render_template('feedback.html', feedback=[], message="No questions were asked.")

    feedback_data = []

    if request.method == 'POST':
        for index, question in enumerate(asked_questions):
            user_answer = request.form.get(f'answer_{index + 1}', "")
            correct_answer = question.get('answer', '')
            question_text = question.get('question', '')

            similarity = check_similarity(correct_answer, user_answer)
            feedback_data.append({
                'question': question_text,
                'user_answer': user_answer,
                'correct_answer': correct_answer,
                'is_correct': similarity >= 0.2,
...
def check_similarity(correct_answer, user_answer):

    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.metrics.pairwise import cosine_similarity

    vectorizer = TfidfVectorizer()
    documents = [correct_answer, user_answer]
